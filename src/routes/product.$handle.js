...
import { Loader2, ChevronRight, ChevronLeft, ChevronDown, Plus, Star, Hammer, ShieldCheck, Truck, Phone, Headphones, Mail, User, ArrowRight, Shield, Moon, CalendarClock, SlidersHorizontal } from "lucide-react";
...
const SPEC_SECTIONS = [
    {
        title: "Algemeen",
        body: (<p>Wandig Full House is een complete cinewall met open vakken, gesloten kastruimte en een centraal tv-vlak. Ontworpen in Nederland, met de hand gebouwd in onze eigen werkplaats en plug &amp; play voorbereid voor jouw woonkamer.</p>),
    },
    {
        title: "Afmetingen",
        body: (<ul className="space-y-1.5">
...
function swatchStyle(name) {
    ...
    if (key.includes("eikenzwart")) {
        return {
            ...
            backgroundImage: "repeating-linear-gradient(94deg, rgba(255,255,255,0.08) 0 1px, transparent 1px 5px), repeating-linear-gradient(2deg, rgba(0,0,0,0.22) 0 2px, transparent 2px 13px), linear-gradient(135deg, #11100f 0%, #26231f 50%, #171615 100%)",
        };
    }
    if (key.includes("eikengrijs")) {
        return {
            backgroundColor: "#9c9b90",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(66,58,48,0.24) 0 1px, transparent 1px 5px), repeating-linear-gradient(6deg, rgba(239,235,218,0.2) 0 2px, transparent 2px 15px), linear-gradient(135deg, #b9b5a7 0%, #98978e 52%, #777970 100%)",
        };
    }
    if (key.includes("walnoot") || key.includes("noten")) {
        return {
            backgroundColor: "#6b3f22",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(37,18,8,0.28) 0 1px, transparent 1px 5px), repeating-linear-gradient(5deg, rgba(211,149,83,0.14) 0 2px, transparent 2px 15px), linear-gradient(135deg, #8b552c 0%, #68401f 54%, #3e2413 100%)",
        };
    }
    if (key.includes("truffel")) {
        return {
            backgroundColor: "#7a5a43",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(48,33,22,0.22) 0 1px, transparent 1px 5px), repeating-linear-gradient(7deg, rgba(205,174,139,0.14) 0 2px, transparent 2px 16px), linear-gradient(135deg, #8b6a50 0%, #72523e 55%, #4f392d 100%)",
        };
    }
    if (key.includes("katoen")) {
        return {
            backgroundColor: "#aeb3b3",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(75,82,82,0.16) 0 1px, transparent 1px 5px), repeating-linear-gradient(8deg, rgba(255,255,255,0.14) 0 2px, transparent 2px 16px), linear-gradient(135deg, #c7cbcb 0%, #aeb3b3 55%, #8e9698 100%)",
        };
    }
    if (key.includes("zandsteen")) {
        return {
            backgroundColor: "#c6a15f",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(104,75,32,0.18) 0 1px, transparent 1px 5px), repeating-linear-gradient(8deg, rgba(255,234,186,0.18) 0 2px, transparent 2px 16px), linear-gradient(135deg, #d8b873 0%, #bd9655 55%, #98723b 100%)",
        };
    }
    if (key.includes("klei")) {
        return {
            backgroundColor: "#bcae9d",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(95,76,58,0.16) 0 1px, transparent 1px 5px), repeating-linear-gradient(8deg, rgba(255,247,230,0.18) 0 2px, transparent 2px 16px), linear-gradient(135deg, #d0c4b3 0%, #b8aa97 55%, #978976 100%)",
        };
    }
    if (key.includes("dofroze") || key.includes("roze")) {
        return {
            backgroundColor: "#c4a29e",
            backgroundImage: "repeating-linear-gradient(94deg, rgba(105,71,70,0.16) 0 1px, transparent 1px 5px), repeating-linear-gradient(8deg, rgba(255,235,230,0.18) 0 2px, transparent 2px 16px), linear-gradient(135deg, #d3b4af 0%, #bd9a95 55%, #9f7b77 100%)",
        };
    }
    if (hasOak && hasBlack) {
        return {
            backgroundColor: "#1f1d1a",
            backgroundImage: "repeating-linear-gradient(86deg, rgba(255,255,255,0.055) 0 1px, transparent 1px 7px), repeating-linear-gradient(6deg, rgba(0,0,0,0.3) 0 2px, transparent 2px 16px), linear-gradient(135deg, #11100f 0%, #24211e 45%, #151413 100%)",
        };
    }
    if (hasOak && hasGrey) {
        return {
            backgroundColor: "#a9aba6",
            backgroundImage: "repeating-linear-gradient(88deg, rgba(70,64,55,0.2) 0 1px, transparent 1px 7px), repeating-linear-gradient(12deg, rgba(255,255,255,0.14) 0 2px, transparent 2px 18px), linear-gradient(135deg, #c8c6bb 0%, #a8a59b 48%, #7e8078 100%)",
        };
    }
    if (hasOak || hasBeige) {
        return {
            backgroundColor: "#c7ad78",
            backgroundImage: "repeating-linear-gradient(88deg, rgba(103,67,31,0.18) 0 1px, transparent 1px 7px), repeating-linear-gradient(10deg, rgba(255,244,211,0.18) 0 2px, transparent 2px 18px), linear-gradient(135deg, #d9bf86 0%, #bd9860 52%, #8d6334 100%)",
        };
    }
    if (hasBrown) {
        return {
            backgroundColor: "#8a572b",
            backgroundImage: "repeating-linear-gradient(88deg, rgba(38,18,8,0.24) 0 1px, transparent 1px 7px), repeating-linear-gradient(9deg, rgba(255,211,155,0.12) 0 2px, transparent 2px 17px), linear-gradient(135deg, #a26734 0%, #7b4824 52%, #422513 100%)",
        };
    }
    return {
        backgroundColor: colorToCss(name),
...
function ProductPage() {
    ...
    if (isLoading) {
        return (<div className="bg-[#faf8f5]">
...
function ProductView({ product }) {
    ...
    const mainGalleryImageRef = useRef(null);
    const galleryContinuationRef = useRef(null);
    ...
    const [expandedVariantOption, setExpandedVariantOption] = useState(null);
    ...
    const activeVariant = useMemo(() => {
        return variants.find((v) => v.selectedOptions.every((o) => selected[o.name] === o.value)) || variants[0];
    }, [variants, selected]);
    ...
    const images = useMemo(() => {
        ...
        const anchorVariants = variants
            ...
            .filter((v) => Boolean(v.color && v.url));
        const selectedAnchorUrls = new Set(anchorVariants
            .filter((v) => v.color === selectedColor && (!positionKey || !selectedPosition || v.position === selectedPosition))
            .map((v) => v.url));
        const boundaryAnchorUrls = new Set(anchorVariants
            .filter((v) => v.color !== selectedColor || (positionKey && selectedPosition && v.position !== selectedPosition))
            .map((v) => v.url));
    ...
    useEffect(() => {
        ...
        const scheduleUpdate = () => {
            if (!frame)
                frame = window.requestAnimationFrame(updateMainImage);
        };
        updateMainImage();
    ...
    return (<div className="bg-[#faf8f5]">
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_490px] lg:gap-10 xl:gap-14">
        ...
        <div className="min-w-0">
          {galleryItems[0] && (<figure className={`overflow-hidden rounded-[6px] lg:sticky lg:top-0 lg:z-0 ${product.handle === "full-house" ? "flex aspect-[5/4] items-center justify-center bg-[#faf8f5]" : ""}`}>
              <img ref={mainGalleryImageRef} src={galleryItems[0].src} alt={galleryItems[0].alt} className={`block origin-center transition-[filter,transform] ease-out [will-change:filter,transform] ${product.handle === "full-house" ? "h-auto w-[76.16%] max-w-none object-contain" : galleryItems[0].square ? "aspect-square w-full object-contain" : "aspect-[4/3] w-full object-cover"}`} loading="eager" fetchPriority="high"/>
            </figure>)}

          <div ref={galleryContinuationRef} className="relative z-10 mt-3 grid grid-cols-2 gap-3 md:mt-4 md:gap-4">
            {galleryItems.slice(1).map((image, index) => (<figure key={`${image.src}-${index + 1}`} className={`${image.full ? "col-span-2" : "col-span-1"} overflow-hidden rounded-[6px]`}>
                <img src={image.src} alt={image.alt} className={`block w-full ${image.square ? "aspect-square object-contain" : "aspect-[4/3] object-cover"}`} loading="lazy"/>
              </figure>))}
          </div>
        </div>
        ...
        <div className="min-w-0 lg:sticky lg:top-3 lg:ml-auto lg:w-[490px] lg:self-start">
          <div className="space-y-3">
            <section className="overflow-hidden rounded-[20px] border border-[#eeeeee] bg-[#fef9f5] shadow-[0_18px_45px_rgba(42,31,22,0.07)]">
              <button type="button" onClick={() => setProductionDetailsOpen((open) => !open)} aria-expanded={productionDetailsOpen} className="flex min-h-[42px] w-full items-center justify-between gap-4 px-4 text-left text-[#071426]">
                <span className="flex items-center gap-2 font-sans text-[14.4px] font-[385] text-[#cdc0b5]" style={{ textShadow: '0 0.55px 0.55px rgba(0,0,0,0.065)' }}>
                  <img src={dutchDesignIcon.url} alt="" aria-hidden="true" className="h-3.5 w-5 shrink-0 object-contain opacity-80"/>
                  Dutch Design
                </span>
              ...
              <div className={`grid transition-[grid-template-rows,opacity] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${productionDetailsOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                <div className="overflow-hidden">
                  <div className="relative min-h-[210px] px-5 pb-8 pt-3 text-[#cdc0b5]">
                    ...
                    <img src={puzzlePiecesImg.url} alt="" aria-hidden="true" className="pointer-events-none absolute bottom-5 right-5 w-[84px] select-none"/>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="mt-[15px] grid grid-cols-1 items-start gap-4 sm:grid-cols-[minmax(0,1fr)_230px]">
                  <div>
                    ...
                    <div className="mt-1 flex items-center text-[#4f5966]/78">
                      <span className="flex items-center gap-0.5">
                        {Array.from({ length: 5 }).map((_, index) => (<Star key={index} className="h-3 w-3 fill-current" strokeWidth={0}/>))}
                      </span>
                  ...
                  <div className="min-w-0 text-right">
                    ...
                    {installmentPrice && (<div className="mt-2 text-[12px] leading-[1.4] text-[#071426]/42">
                        ...
                        <p className="mt-1 flex items-baseline justify-end gap-2">
                          <strong className="text-[14px] font-bold leading-none text-[#071426]" style={{ fontFamily: '"Klarna Headline", "Circular-Regular", sans-serif' }}>
                            Klarna.
                          </strong>
              ...
              {hasOptions && (<div className="mt-4 space-y-2">
                ...
                if (isPosition || isTvSize) {
                    return (<div key={opt.name} className="overflow-hidden rounded-[12px] border border-[#eeeeee]">
                          <button type="button" onClick={() => setExpandedVariantOption((current) => current === opt.name ? null : opt.name)} aria-expanded={optionExpanded} className="flex min-h-[52px] w-full items-center gap-2 px-3 text-left">
                          ...
                          <div className={`grid transition-[grid-template-rows,opacity] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${optionExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                            ...
                            return (<button key={choice.label} type="button" onClick={() => setSelected((current) => ({ ...current, [opt.name]: choice.value }))} aria-pressed={active} className={`h-10 rounded-[8px] border bg-[#f8f6f4] px-2 text-[12px] font-medium text-[#071426] transition-[border-color,box-shadow,background-color,transform] duration-300 ease-out hover:bg-[#f3ece6] active:scale-[0.98] ${active ? "border-[#ff5a00] bg-[#fff8f3] shadow-[0_0_0_2px_rgba(255,90,0,0.18)]" : "border-[#eeeeee] shadow-none"}`}>
                                      {choice.label}
                                    </button>);
                        })}
                              </div>
                ...
                return (<div key={opt.name} className="grid min-h-[52px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-[12px] border border-[#eeeeee] px-3">
                        {isColor ? (<>
                            ...
                            return (<button key={value} type="button" onClick={() => setSelected((current) => ({ ...current, [opt.name]: value }))} title={value} aria-label={`Kleur ${value}`} aria-pressed={active} className={`h-9 w-9 shrink-0 rounded-full border-2 p-[2px] transition-transform hover:scale-105 active:scale-95 ${active ? "border-[#ff5a00]" : "border-transparent"}`}>
                            ...
                            <select value={selected[opt.name] || ""} onChange={(event) => setSelected((current) => ({ ...current, [opt.name]: event.target.value }))} className="h-10 w-full appearance-none bg-transparent pr-7 text-[14px] text-[#071426] outline-none">
              ...
              <Button onClick={handleAdd} disabled={isLoading || !activeVariant?.availableForSale} className="group mt-3 h-12 w-full translate-y-0 overflow-hidden rounded-full bg-[#ff5a00] px-6 text-sm font-bold text-white shadow-none transition-colors hover:translate-y-0 hover:bg-[#e95100] hover:shadow-none active:translate-y-0 active:scale-100">
                {isLoading ? <Loader2 className="h-4 w-4 animate-spin"/> : activeVariant?.availableForSale ? (<span className="relative block h-full w-full overflow-hidden">
            ...
            <section className="overflow-hidden rounded-[20px] border border-[#eeeeee] bg-white p-3 shadow-[0_14px_34px_rgba(42,31,22,0.05)]">
              <div className="mb-3 flex items-center justify-between px-1">
                ...
                <div className="flex gap-1.5">
                  <button type="button" aria-label="Vorige voordelen" onClick={() => scrollBenefits(-1)} disabled={benefitsScrollState.atStart} className="flex h-8 w-8 items-center justify-center rounded-full text-[#071426] transition-opacity disabled:text-[#071426]/25">
                    <ChevronLeft className="h-5 w-5" strokeWidth={1.5}/>
                  </button>
                  <button type="button" aria-label="Volgende voordelen" onClick={() => scrollBenefits(1)} disabled={benefitsScrollState.atEnd} className="flex h-8 w-8 items-center justify-center rounded-full text-[#071426] transition-opacity disabled:text-[#071426]/25">
              ...
              <div ref={benefitsScrollerRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {PRODUCT_BENEFITS.map((benefit) => (<article key={benefit.title} className="relative h-[195px] min-w-[150px] snap-start overflow-hidden rounded-[13px] bg-[#eee4dc] shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
      ...
      <section className="mt-10 md:mt-16">
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-24">
              <div>
                ...
                <div className="relative mt-3" style={{ paddingTop: "32px", paddingBottom: "28px", paddingLeft: "16px", paddingRight: "32px" }}>
                  {/* Width dimension (top) */}
                  ...
                  <img src={cinewallSchema.url} alt="Schematische weergave Wandig Full House, 240 cm breed en 180 cm hoog" className="relative block h-auto w-full object-contain" style={{ background: "transparent" }} loading="lazy"/>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            ...
            return (<div key={section.title} className="group border-b border-[#071426]/10">
                      <button type="button" onClick={() => setOpenSpecs((current) => {
                    const opening = !current[section.title];
                    return opening ? { [section.title]: true } : {};
                })} aria-expanded={isOpen} className="flex w-full cursor-pointer items-center justify-between gap-4 py-5 text-left text-[15px] font-semibold text-[#071426]">
                        <span>{section.title}</span>
                        <span className={`flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full border border-[#ef7027]/40 text-[#ef7027] transition-transform duration-300 ease-out ${isOpen ? "rotate-45" : "rotate-0"}`}>
      ...
      <div className={`pointer-events-none fixed bottom-5 left-5 z-50 transition-all duration-300 ease-out ${showOrderWidget ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`}>
        <button type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label={`Bestel ${displayTitle} en kies je kleur`} className={`group flex items-center gap-4 rounded-[18px] py-2.5 pl-5 pr-2.5 shadow-[0_14px_35px_rgba(42,31,22,0.18)] transition-transform duration-200 hover:-translate-y-0.5 ${showOrderWidget ? "pointer-events-auto" : ""}`} style={{ background: "linear-gradient(105deg, #f9cfa8 0%, #f5a87a 55%, #ef9464 100%)" }}>
...
function UniqueSection() {
    ...
    return (<section className="mt-12 md:mt-20">
      ...
      <div ref={scrollerRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} className={`-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth px-4 pb-2 md:gap-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${isDragging ? "cursor-grabbing select-none" : "cursor-grab"}`}>
        {UNIQUE_CARDS.map((card) => (<article key={card.title} className="relative h-[440px] w-[280px] shrink-0 snap-center overflow-hidden rounded-[18px] bg-[#f2ece3] md:h-[520px] md:w-[360px]">
            <img src={card.image} alt={card.title} className="absolute inset-0 h-full w-full object-cover" loading="lazy" draggable={false}/>
            {!card.noGradient && (<div className={`absolute inset-x-0 top-0 px-6 pb-10 pt-6 md:px-7 md:pt-7 ${card.darkGradientLower
            ...
            <div className="absolute inset-x-0 top-0 px-6 pb-10 pt-6 md:px-7 md:pt-7">
              {card.eyebrow && (<p className={`mb-2 text-[11px] font-semibold uppercase tracking-[0.15em] ${card.lightText ? "text-white/70" : "text-[#071426]/60"}`}>{card.eyebrow}</p>)}
              <h3 className={`text-[20px] md:text-[22px] font-bold leading-tight ${card.lightText ? "text-white" : "text-[#071426]"}`}>{card.title}</h3>
              {card.body && (<p className={`mt-2 text-[13px] leading-relaxed ${card.lightText ? "text-white" : "text-[#071426]"}`}>{card.body}</p>)}
            </div>
          </article>))}
      </div>

      <div className="mt-6 flex justify-center gap-2">
        {UNIQUE_CARDS.map((_, i) => (<button key={i} type="button" aria-label={`Ga naar kaart ${i + 1}`} onClick={() => scrollToIndex(i)} className={`h-2 rounded-full transition-all ${activeIndex === i ? "w-6 bg-[#071426]" : "w-2 bg-[#071426]/25"}`}/>))}
      </div>
    </section>);
}
function BeforeAfterSection() {
    ...
    return (<section className="mt-12 md:mt-20">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
        ...
        {/* Before/After slider */}
        <div className="lg:col-span-7">
          <div className="relative w-full overflow-hidden rounded-2xl select-none touch-none shadow-[0_8px_30px_rgba(0,0,0,0.08)] ring-1 ring-[#071426]/5" style={{ aspectRatio: "4 / 3" }} ref={containerRef} onPointerDown={(e) => {
            draggingRef.current = true;
            setFromClientX(e.clientX);
        }}>
            <img src={afterFullHouseAsset.url} alt="Woonkamer na Full House" className="absolute inset-0 h-full w-full object-cover" draggable={false}/>
            <div className="absolute inset-0 overflow-hidden" style={{ clipPath: `inset(0 ${100 - position}% 0 0)` }}>
              <img src={beforeFullHouseAsset.url} alt="Woonkamer voor Full House" className="absolute inset-0 h-full w-full object-cover" draggable={false}/>
            </div>
            ...
            <div className="absolute top-0 bottom-0 w-[2px] bg-white shadow-[0_0_12px_rgba(0,0,0,0.35)] pointer-events-none" style={{ left: `${position}%`, transform: "translateX(-50%)" }}/>
            <button type="button" aria-label="Sleep om te vergelijken" className="absolute top-1/2 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white shadow-[0_4px_16px_rgba(0,0,0,0.25)] cursor-ew-resize" style={{ left: `${position}%` }} onPointerDown={(e) => {
            e.stopPropagation();
            draggingRef.current = true;
        }}>
...
function PuzzleCornerIcon({ className }) {
    ...
    return (<section className="mt-12 md:mt-20">
      ...
      <div className="-mx-4 overflow-x-auto scrollbar-hide md:mx-0">
        <div className="flex h-[420px] gap-3 px-4 md:h-[560px] md:gap-4 md:px-0">
          {CUSTOMER_GALLERY_COLUMNS.map((col, colIndex) => (<div key={colIndex} className={`flex h-full shrink-0 flex-col gap-3 md:gap-4 ${col.width}`}>
              {col.items.map((image, i) => (<figure key={i} className="group relative min-h-0 flex-1 overflow-hidden rounded-[14px]">
                  <img src={image.src} alt={image.alt} className="h-full w-full object-cover" loading="lazy" draggable={false}/>
                  <button type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Bestel Full House" className="group/pill absolute bottom-2 left-2 flex h-7 items-center gap-0 rounded-full bg-[#ff843a] pl-1 pr-1 text-white shadow-[0_2px_8px_rgba(0,0,0,0.15)] transition-[padding,gap] duration-300 ease-out hover:gap-2 hover:pl-3 hover:pr-2 md:bottom-3 md:left-3 md:h-8">
...
function BuiltToLastSection() {
    ...
    return (<section className="bg-gradient-to-b from-[#faf8f5] to-white pt-12 md:pt-20 pb-12 md:pb-20">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        ...
        <div ref={scrollerRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag} onPointerCancel={endDrag} onPointerLeave={endDrag} className={`-mx-5 flex gap-3 overflow-x-auto scrollbar-hide px-5 md:mx-0 md:gap-4 md:px-0 ${isDragging ? "cursor-grabbing select-none" : "cursor-grab"}`} style={{ scrollSnapType: "x mandatory" }}>
          {BUILT_TO_LAST_CARDS.map((card, i) => (<article key={i} className="relative shrink-0 overflow-hidden rounded-[18px] w-[280px] h-[280px] md:w-[360px] md:h-[360px]" style={{ scrollSnapAlign: "start", backgroundColor: card.bg }}>
              {card.imageUrl && (<img src={card.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover"/>)}
              <div className={`relative flex h-full flex-col p-6 md:p-7 ${card.textColor
                ? ""
                : card.tone === "light"
                    ? "text-[#071426]"
                    : "text-white"}`} style={{ color: card.textColor }}>
                <div className="text-[12px] md:text-[13px] font-normal" style={{ color: card.textColor, opacity: card.textColor ? 1 : undefined }}>
                  {card.eyebrow}
                </div>
                <h3 className="mt-2 whitespace-pre-line text-[18px] md:text-[22px] font-bold leading-[1.15]">
                  {card.title}
                </h3>
                <p className="mt-3 max-w-[240px] text-[13px] md:text-[14px] leading-relaxed font-light" style={{ color: card.textColor, opacity: card.textColor ? 1 : undefined, maxWidth: card.bodyMaxWidth }}>
                  {card.body}
                </p>
              </div>
            </article>))}
...
function FaqSection() {
    ...
    const renderItem = (item, i, isRight) => {
        const isOpen = isRight ? openRight === i : openLeft === i;
        const setOpen = isRight ? setOpenRight : setOpenLeft;
        return (<div key={`${isRight ? "r" : "l"}-${i}`} className="rounded-[14px] bg-white p-4 shadow-[0_2px_10px_rgba(42,31,22,0.06)] md:p-5">
        <button type="button" onClick={() => setOpen(isOpen ? null : i)} aria-expanded={isOpen} className="flex w-full cursor-pointer items-center justify-between gap-4 text-left">
          <span className="text-[14px] font-[500] leading-snug text-[#071426] md:text-[15px]">
            {item.question}
          </span>
          <span className={`flex h-[24px] w-[24px] shrink-0 items-center justify-center rounded-full border border-[#cdc0b5] text-[#071426] transition-transform duration-300 ease-out ${isOpen ? "rotate-45" : "rotate-0"}`}>
    ...
    return (<section className="bg-white pb-12 pt-0 md:pb-20">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        <div className="mb-6 text-center md:mb-10">
...
function ReviewsSection() {
    ...
    return (<section className="bg-[#fff7ef] py-10 md:py-16">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        <div className="mb-6 flex items-end justify-between gap-4 md:mb-10">
          ...
          <div className="hidden gap-2 md:flex">
            <button type="button" onClick={() => scroll(-1)} aria-label="Vorige" className="flex h-10 w-10 items-center justify-center rounded-full border border-[#f4f2ee] bg-white text-[#071426] transition hover:bg-[#faf8f5]">
              ‹
            </button>
            <button type="button" onClick={() => scroll(1)} aria-label="Volgende" className="flex h-10 w-10 items-center justify-center rounded-full border border-[#f4f2ee] bg-white text-[#071426] transition hover:bg-[#faf8f5]">
              ›
            </button>
          </div>
        </div>
      ...
      <div ref={scrollerRef} className="mx-auto flex max-w-[1400px] snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth px-5 md:px-10 pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-pl-5 md:scroll-pl-10">
        {REVIEWS.map((r, i) => (<article key={i} className="flex w-[280px] shrink-0 snap-start flex-col justify-between rounded-[14px] bg-white p-5 shadow-[0_2px_10px_rgba(42,31,22,0.06)] md:w-[340px]">
            <div className="pb-5">
              <div className="flex items-center gap-1 text-[#ef7027]">
                {Array.from({ length: 5 }).map((_, idx) => (<span key={idx} className={idx < r.rating ? "text-[#ef7027]" : "text-[#e5ded4]"}>★</span>))}
              </div>
...
function NewsletterContactSection() {
    ...
    return (<section className="bg-[#ede7e0]">
          ...
          <div>
            ...
            <form onSubmit={onSubmit} className="mt-6 max-w-[430px]" noValidate>
              <div className="flex overflow-hidden rounded-[8px] border border-[#e7ded4] bg-white">
                ...
                <input id="newsletter-email" type="email" required autoComplete="email" value={email} onChange={(e) => { setEmail(e.target.value); if (status !== "idle")
        setStatus("idle"); }} placeholder="Voer je e-mailadres in" className="h-[46px] min-w-0 flex-1 bg-white px-4 text-[14px] tracking-[0.01em] text-[#071426] placeholder:text-[#071426]/40 focus:outline-none"/>
                <button type="submit" disabled={status === "loading"} className="inline-flex h-[46px] shrink-0 items-center justify-center gap-2 rounded-[8px] bg-[#ef7027] px-5 text-[14px] font-[500] tracking-[0.03em] text-white transition hover:brightness-95 disabled:opacity-60">
              ...
              <label className="mt-4 flex cursor-pointer items-start gap-3">
                <input type="checkbox" checked={acceptedTerms} onChange={(e) => { setAcceptedTerms(e.target.checked); if (status !== "idle")
        setStatus("idle"); }} className="mt-0.5 h-4 w-4 shrink-0 cursor-pointer rounded-[3px] border-[#cdc0b5] text-[#ef7027] focus:ring-[#ef7027]"/>
              ...
              {message && (<p className={`mt-3 text-[13px] tracking-[0.01em] ${status === "success" ? "text-[#2d6a3e]" : "text-[#b3341c]"}`}>{message}</p>)}
            </form>
          </div>

          <div className="grid gap-8 sm:grid-cols-3 sm:gap-6">
            {contactItems.map(({ icon: Icon, label, sub, value }) => (<div key={label}>
...
function ShieldOnlyIcon({ className, strokeWidth = 1.5 }) {
    return (<div className={className}>
      <Shield className="h-full w-full" strokeWidth={strokeWidth}/>
    </div>);
}
function ShieldMoonIcon({ className, strokeWidth = 1.5 }) {
    return (<div className={className}>
...
function TrustBannerSection() {
    ...
    return (<section className="bg-[#ffffff]">
      <div className="mx-auto max-w-[1400px] px-5 py-6 md:px-10 md:py-8">
        <div className="flex flex-col divide-y divide-[#e5e5e5] sm:flex-row sm:divide-y-0 sm:divide-x sm:divide-[#e5e5e5]">
          {items.map(({ icon: Icon, label }) => (<div key={label} className="flex flex-1 items-center justify-center gap-3 py-4 sm:px-4 sm:py-0">
